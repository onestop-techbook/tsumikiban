...
その後しばらく時を経て、野良ハック活動を始めてワインIoT（TWE-Liteと複数のセンサーデバイスを使用して電池駆動で屋外使用）の案件で、P板.comで基板を外注しました。この時に初めてガーバーファイル を覚えます。（基板の自作が「安価で楽（ラク）」へ記憶の上書き)
自作基板遍歴
