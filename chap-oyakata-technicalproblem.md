...
### 要素が増えるとムズイ
単純に要素が増えと、加速度的に難しくなります。もちろんそれぞれの構成要素について知らなければならない情報が増え、その調査をしようにも情報が断片的になってしまいます。また、要素が増えると、途中経路のどこで失敗しているのかの解析が難しくなります。
...
要素が増えた時にその要素を基板上などにまとめて実装すると総当りで検証をする必要があり、チェックが加速度的に大変になりますが、これに対する対抗策は回路を適切な粒度で「モジュール化」し、そのモジュールごとに完成させた上でトータルで動くようにする　というアプローチになります。
